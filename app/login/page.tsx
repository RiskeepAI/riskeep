...
export default async function LoginPage({
  ...
  const plan = params.plan as 'monthly' | 'yearly' | undefined

  const planLabel = plan === 'yearly' ? 'Anual (249€/año)' : plan === 'monthly' ? 'Mensual (29€/mes)' : null
      ...
      <div className="w-full max-w-md space-y-8">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold text-white">
            {planLabel ? 'Inicia sesión para suscribirte' : 'Bienvenido de vuelta'}
          </h1>
          <p className="text-slate-400 text-sm">
            {planLabel ? `Plan seleccionado: ${planLabel}` : 'Accede a tu área de cliente'}
          </p>
        </div>
        ...
        {planLabel && (
          <div className="flex items-center gap-3 p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-300 text-sm">
            <span>🔒</span>
            <span>Inicia sesión o regístrate para completar tu suscripción.</span>
          </div>
        )}
